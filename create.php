...
try {
    $stmt = $conn->prepare("SELECT id, name FROM customers WHERE cid = ?");
    ...
    $stmt = $conn->prepare("SELECT id, item_name FROM item_masters WHERE cid = ?");
    $stmt->bind_param("i", $cid);
    $stmt->execute();
...
// Predefined colors and cups
$colors = ['Skin', 'Black', 'White', 'Red', 'Baby Pink', 'Dark Pink', 'Grey', 'Royal Blue'];
$cups = ['A', 'B', 'C', 'D', 'DD', 'E'];
    ...
    try {
        ...
        $stmtDetail = $conn->prepare("INSERT INTO order_item_detail 
            (order_id, order_no, item_id, color, cup, size_32, size_34, size_36, size_38, size_40, size_42, 
             size_44, size_46, size_48, size_50, total_qty, cid) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        foreach ($items_data as $itemData) {
            ...
            foreach ($rows as $row) {
                ...
                $stmtDetail->bind_param("iiissiiiiiiiiiiii", 
                    $orderId, $orderNo, $item_id, $color, $cup,
                    $size_32, $size_34, $size_36, $size_38, $size_40, $size_42,
                    $size_44, $size_46, $size_48, $size_50, $row_total, $cid
                );
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <label class="form-label mb-0 required">Article Details (Color, Cup & Sizes)</label>
                    <button type="button" class="btn btn-sm btn-add-row" onclick="addRow(${itemIndex})">
                        <i class="bi bi-plus-lg"></i> Add Row
                    </button>
                </div>
                <div id="rows-${itemIndex}" class="mt-3"></div>
            </div>
...
function addRow(itemIdx) {
    ...
    <div class="row-detail" data-row="${rowId}" id="row-${itemIdx}-${rowId}">
        <div class="row g-2 mb-3">
            ...
            <div class="col-md-5 col-lg-7 d-flex align-items-end">
